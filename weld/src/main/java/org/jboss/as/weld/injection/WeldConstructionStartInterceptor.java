...
import org.jboss.weld.construction.AroundConstructCallback;
import org.jboss.weld.construction.ConstructionHandle;
...
public class WeldConstructionStartInterceptor implements Interceptor {
    ...
    private <T> void setupAroundConstructCallback(CreationalContext<T> ctx, final InterceptorContext context) {
        ...
        ctxImpl.setCdiLifecycleInterceptionSuppressed(true); // Weld will not try to invoke any lifecycle interceptors on this instance

        ctxImpl.addAroundConstructCallback(new AroundConstructCallback<T>() {
